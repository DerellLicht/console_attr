...
//  the previous report.
//*************************************************************
char *registry_iface::regif_get_system_message(void)
{
   static char msg[261] ;
   int slen ;
...
//*****************************************************************
void registry_iface::strip_newlines(char *str)
{
   int slen = strlen(str) ;
   while (1) {
      if (slen == 0)
         break;
      slen-- ; //  change length to last-char index
      if (*(str+slen) == '\n'  ||  *(str+slen) == '\r')
          *(str+slen) = 0 ;
      else
         break;
...
int registry_iface::WriteRegifProfileString(char *field, char *value)
{
   ...
   if (!found) {
      wsprintf(rtempstr, "%s=%s", field, value) ;
      strncpy(ini_read_bfr[idx], rtempstr, sizeof(ini_read_bfr[idx])) ;
      idx++ ;
   }
...
{
   // char rtempstr[81] ;
   // sprintf(rtempstr, "grp=%s, name=%s, lstr=%s, ininame=%s", 
   //    grp, name, lstr, ininame) ;
   // show_message(NULL, rtempstr) ;

   if (WriteRegifProfileString(name, lstr) == 0) {
      // if (hwndStatus != 0) {
      //    wsprintf(pbfr, "INI write %s %s: %s", grp, name, regif_get_system_message()) ;
